import java.lang.reflect.Array;
import java.util.ArrayList;
import java.util.List;

public class mcoloringgraph {
    private static boolean issafe(int node, List < Integer > [] G, int[] color, int n, int col) {
        for (int it: G[node]) {
            if (color[it] == col) return false;
        }
        return true;
    }

    private static boolean solve(int node ,ArrayList<Integer>[] G,int[] color,int n , int m ){
        //base case 
        if(node==n) return true;

        for(int i=1 ; i<=m;i++){
            if(issafe(node , G , color,n,i)){
                color[node]=i;
                if(solve(node+1, G, color, n, m)==true) return true;
                color[node]=0;
            }
        }
        return false;
    }
    private static boolean graphcoloring(ArrayList<Integer>[] G , int [] color,int i,int m){
        int n=G.length;

        if(solve(i,G,color,n,m)==true) return true;

        return false;
    }
    public static void main(String[] args) {
        int n=4;
        ArrayList<Integer>[] G=new ArrayList[n];

        for(int i=0;i<n;i++){
            G[i]=new ArrayList<>();
        }
        G[0].add(1);
        G[0].add(3);
        G[0].add(2);
        G[1].add(0);
        G[1].add(2);
        G[2].add(1);
        G[2].add(3);
        G[2].add(0);
        G[3].add(2);
        G[3].add(0);

        int m=3;

        // making the color array 
        int[] color = new int[n];

        boolean ans = graphcoloring(G,color,0,m);

        if(ans==true){
            System.out.println(1);
        }
        else{
            System.out.println(-1);
        }
    }
}
